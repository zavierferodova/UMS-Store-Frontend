...
import { OpenCashierBookDialog } from './components/OpenCashierBookDialog';
import { CloseCashierBookDialog } from './components/CloseCashierBookDialog';
import { Banknote, CreditCard, Percent, Search, Ticket } from 'lucide-react';
import { Input } from '@/components/ui/input';
import { FilterDialog } from './components/FilterDialog';
import Link from 'next/link';

function CashierBookPageContent() {
  const {
    status,
    transactions,
    pagination,
  ...
  return (
    <div className="flex flex-col gap-4">
      {activeCashierBook && cashierBookStats && (
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle>Buku Kasir</CardTitle>
              <CardDescription>
                {activeCashierBook
                  ? 'Daftar transaksi pada buku kasir aktif'
                  : 'Buka buku kasir untuk melihat transaksi'}
              </CardDescription>
            </div>
            {activeCashierBook ? (
              <CloseCashierBookDialog onCloseAction={handleCloseCashierBook} />
            ) : (
              <OpenCashierBookDialog onOpenAction={handleOpenCashierBook} />
            )}
          </div>
          {activeCashierBook && (
            <div className="mt-4 flex w-full flex-col justify-end gap-2 md:w-auto md:flex-row">
        ...
        <CardContent>
          {status === PageStatus.LOADING ? (
            ...
            <EmptyDisplay
              ...
              description="Silahkan buka buku kasir terlebih dahulu."
            />
          ) : isEmpty ? (
            <EmptyDisplay
              title="Tidak ada data"
...
export default function CashierBookPage() {
  return <CashierBookPageContent />;
}
