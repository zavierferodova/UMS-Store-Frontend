...
export const formSchema = z.object({
  ...
  price: z.number().min(1, { message: "Harga produk tidak boleh kosong" }),
  category: z.string().min(1, { message: "Kategori produk tidak boleh kosong" }),
  images: z.array(z.any())
    .min(1, { message: "Gambar tidak boleh kosong" })
    .refine(
      (files) => files.every((file: ImageFile) => file?.file?.size <= 5 * 1024 * 1024),
      { message: "Ukuran gambar maksimal 5MB" }
    )
    .refine(
      (files) => files.every((file: ImageFile) => 
