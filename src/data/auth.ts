...
class AuthData implements IAuthData {
  ...
  async updateUser(user: UpdateUserParams): Promise<User | null> {
    try {
  ...
  async updatePassword(newPassword: string, passwordConfirm: string): Promise<boolean> {
    try {
      const session = await this.getAuthSession();
